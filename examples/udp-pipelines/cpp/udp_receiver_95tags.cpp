 ...
 *
 * @details
 * This example listens for a UDP MPEG-TS stream, demuxes video and KLV,
 * decodes KLV JSON, and prints tag values with names and units sourced
 * from `data/stanag4609_tags.ini`.
 *
 * @section gstklv_udp_receiver_pipeline Pipeline Topology
 * @dot
...
void
on_pad_added(GstElement *, GstPad *pad, gpointer user_data)
{
...
main(int argc, char **argv)
{
  ...
  GOptionEntry entries[] = {
    {"host", 0, 0, G_OPTION_ARG_STRING, &host_opt, (gchar *)"Listen host", (gchar *)"0.0.0.0"},
    {"port", 0, 0, G_OPTION_ARG_INT, &opt.port, (gchar *)"UDP port", (gchar *)"5000"},
    {"headless",
     0,
     0,
     G_OPTION_ARG_NONE,
  ...
  GOptionContext *ctx = g_option_context_new(" - C++ UDP receiver with MISB ST 0601.8 tags");
  ...
  std::cout << "\n" << std::string(100, '=') << "\n";
  std::cout << "UDP RECEIVER (C++) - MISB ST 0601.8 KLV TAGS\n";
  ...
  if (!pipeline || !udpsrc || !demux || !queue_video || !h264parse || !decoder || !videoconvert ||
  ...
  g_object_set(udpsrc, "address", opt.host.c_str(), "port", opt.port, "caps", src_caps, nullptr);
